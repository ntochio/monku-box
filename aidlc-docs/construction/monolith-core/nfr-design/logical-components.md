...
物理インフラの詳細（VPC、マシンサイズ等）は対象外とし、**論理ブロック**とその責務・接続を定義する。NFR は「単一デプロイ・軽量永続・最小観測性」を前提とする。クライアント↔モノリス間の API は **`requirement-verification-questions.md`（インフラ構築および通信方式）** に従い **JSON を中心**とし、SOAP/gRPC 等の厳密標準化は PoC では行わない。
